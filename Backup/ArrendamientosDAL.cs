...
namespace Arrendamientos
{
    public class ArrendamientosDAL
    {
        ...
        public static List<ArrendamientosVO> ListaPaso2(string Numero,string Empresa,string Arrendadora,string Equipo,string Proveedor,string Condicionesdepago,string Estatus)
        {
            ...
            if (Numero != "") Q = Q + " and Numero='" + Numero + "'";

            if (Empresa != "") Q = Q + " and Empresa='" + Empresa + "'";
            if (Arrendadora != "") Q = Q + " and Arrendadora='" + Arrendadora + "'";
            if (Equipo != "") Q = Q + " and Equipo='" + Equipo + "'";
            if (Proveedor != "") Q = Q + " and Proveedor='" + Proveedor + "'";
            if (Condicionesdepago != "") Q = Q + " and Condicionesdepago='" + Condicionesdepago + "'";
            if (Estatus != "") Q = Q + " and Estatus='" + Estatus + "'";
            try
            {
                string Query = Q;
                if (conn.State != ConnectionState.Open) conn.Open();
        ...
        {
            SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["Conn"].ConnectionString);
            string Q = "select * from Arrendamientos_docs where idArrendamiento='"+id+"'";
            try
            {
                string Query = Q;
                if (conn.State != ConnectionState.Open) conn.Open();
